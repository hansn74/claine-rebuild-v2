...
function useOptimisticEmail(emailId: string) {
  ...
  const { result: emails } = useRxData<EmailDocument>(
    'emails',
    (collection) => collection.findOne(emailId)
  );
...
function useAutoSync() {
  ...
  useEffect(() => {
    const handleOnline = async () => {
      ...
      const pendingEmails = localEmails.filter(
        (email) => email.syncStatus === 'pending'
      );
...
async function handleRetry(action: QueuedAction) {
  ...
  const delay = Math.min(
    INITIAL_DELAY * Math.pow(BACKOFF_MULTIPLIER, action.retries),
    MAX_DELAY
  );
...
function useDeduplication() {
  ...
  const deduplicateQueue = async () => {
    ...
    const grouped = queue.reduce((acc, action) => {
    ...
    for (const [key, actions] of Object.entries(grouped)) {
      if (actions.length > 1) {
        ...
        await Promise.all(
          toRemove.map((action) =>
            db.collections.offlineQueue.findOne(action.id).remove()
          )
        );
...
function detectQueueConflicts(queue: QueuedAction[]): QueuedAction[][] {
  ...
  // Group by emailId
  const byEmail = queue.reduce((acc, action) => {
  ...
  const syncAll = async () => {
    ...
    for (const localEmail of localEmails) {
      ...
      if (serverEmail) {
        // Check for conflict
        if (
          localEmail.starred !== serverEmail.starred ||
          localEmail.read !== serverEmail.read
        ) {
