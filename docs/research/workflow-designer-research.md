...
const nodePalette = {
  ...
  'Conditions': [
    ...
    { type: 'condition.subject_contains', label: 'Subject Contains', description: 'Check subject text' }
  ],
  'Actions': [
...
class EmailConditionService {
  ...
  private evaluateHasLabel(
    config: HasLabelConditionConfig,
    thread: Thread
  ): boolean {
    const threadLabelIds = thread.labels.map(l => l.id);

    if (config.matchAll) {
      return config.labelIds.every(labelId =>
        threadLabelIds.includes(labelId)
      );
    } else {
      return config.labelIds.some(labelId =>
        threadLabelIds.includes(labelId)
      );
    }
  }

  private evaluateFromSender(
    config: FromSenderConditionConfig,
    message: Message
  ): boolean {
    const fromEmail = message.from.address.toLowerCase();
...
const subject = templateParser.parse(
  "Re: {{thread.subject}} - Follow up",
  {
...
class WorkflowExecutionEngine {
  ...
  private calculateRetryDelay(
    attempt: number,
    config?: ErrorHandlerConfig['retryConfig']
  ): number {
...
class WorkflowCompiler {
  compile(visual: WorkflowVisualData): WorkflowExecutionData {
    // 1. Find trigger node
    const triggerNode = visual.nodes.find(n =>
  ...
  private buildExecutionGraph(
    visual: WorkflowVisualData
  ): ExecutionNode[] {
    return visual.nodes.map(node => {
      // Find outgoing edges
      const outgoingEdges = visual.edges.filter(e =>
        e.source === node.id
      );
...
export class WorkflowExecutionEngine {
  ...
  async execute(
    ...
    try {
      // 1. Evaluate triggers
      const shouldRun = await this.evaluateTriggers(
        workflow,
        context
      );
  ...
  private async executeNode(
    ...
    try {
      ...
      if (node.type.startsWith('condition.')) {
        const conditionMet = await this.conditionService.evaluate(
          node.config,
          context
        );
        result = { conditionMet };
        ...
        result = await this.actionService.execute(
          node.config,
          context
        );
  ...
  private async evaluateTriggers(
    ...
    for (const trigger of workflow.executionData.triggers) {
      const matches = await this.triggerService.evaluate(
        trigger,
        context
      );
