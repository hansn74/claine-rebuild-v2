...
export function useSmartShortcut(
  ...
  const activeConfig = configs.find(c =>
    c.scope === activeScope || c.scope === 'global'
  );
...
export function useOptimizedShortcut(
  keys: string,
  callback: () => void,
...
export function useShortcutCleanup(
  register: () => () => void,
  deps: any[]
) {
...
5. Implement selection shortcuts (*, *, +a, +n)
...
export function measureShortcutPerformance(
  shortcut: string,
  callback: () => void
) {
  return () => {
    ...
    if (end - start > 16) {
      console.warn(
        `Shortcut "${shortcut}" took ${(end - start).toFixed(2)}ms (target: <16ms)`
      );
    }
  };
}
