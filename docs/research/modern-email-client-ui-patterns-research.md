...
<div
  className="group relative flex items-center gap-4 px-4 py-3 hover:bg-accent transition-colors"
>
...
function getFileIcon(mimeType: string) {
  ...
  if (mimeType.includes('zip') || mimeType.includes('archive')) return <Archive className="h-6 w-6" />;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={cn(
        ...
        isExpanded
          ? "max-w-screen h-screen m-0 rounded-none"
          : "max-w-2xl"
      )}>
                ...
                <CardContent className="pt-6">
                  ...
                  <div className="flex gap-2 mt-4">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => insertIntoCompose(response)}
                    >
          ...
          <Card className="p-8 text-center max-w-md">
            <CardContent>
              ...
              <p className="text-muted-foreground">
                You've cleared your inbox. Time to celebrate!
              </p>
