...
class QuotaTracker {
  ...
  async waitForQuota(quotaCost) {
    while (!this.canMakeRequest(quotaCost)) {
      ...
      console.log(`Quota: ${currentUsage}/${this.maxQuota} (need ${quotaCost}), waiting ${waitTime}ms`);
...
async function syncMessages(gmail, messageIds) {
  ...
  console.log(`Completed in ${duration}ms (${(messageIds.length / (duration / 1000)).toFixed(0)} msg/sec)`);
...
class ParallelSyncEngine {
  ...
  async fetchThreadsParallel(threadIds) {
    ...
    const results = await Promise.all(
      batches.map(batch => this.fetchThreadBatch(batch))
    );
    return results.flat();
  }
...
class CheckpointedSyncEngine {
  ...
  async syncWithCheckpoints(userId, messageIds) {
    ...
    const remainingIds = messageIds.filter(id =>
      !checkpoint.completedIds.has(id) && !checkpoint.failedIds.has(id)
    );
...
class OptimisticUpdateManager {
  ...
  async markAsRead(userId, messageId) {
    ...
    await this.storage.updateMessageOptimistic(userId, messageId, {
      labelIds: ['UNREAD']
    }, updateId, 'remove');
  ...
  async flush() {
    ...
    await Promise.allSettled(
      pendingIds.map(id => this.executeUpdate(id))
    );
  }
}
```
...
class PrioritySyncEngine {
  ...
  filterChangesByLabels(changes, labelIds) {
    ...
    return changes.filter(change => {
      ...
      return messages.some(msg =>
        msg.labelIds?.some(label => labelIds.includes(label))
      );
    });
  }
  ...
  emitProgress(syncStatus) {
    ...
    console.log(`Sync progress: ${progressPercent.toFixed(0)}% (${completedTiers}/${totalTiers} tiers)`);
  }
}
```
...
class SparkStyleInbox {
  ...
  isNotification(message) {
    ...
    return notificationPatterns.some(pattern =>
      message.from.toLowerCase().includes(pattern)
    );
  }
...
class PredictivePrefetcher {
  ...
  async generatePredictions() {
    ...
    predictions.push(...recentUnread.map(msg => ({
      messageId: msg.id,
    ...
    predictions.push(...messagesFromFrequent.map(msg => ({
      messageId: msg.id,
    ...
    if (currentThread) {
      ...
      predictions.push(...threadMessages.map(msg => ({
        messageId: msg.id,
    ...
    predictions.push(...nextInboxMessages.map(msg => ({
      messageId: msg.id,
      reason: 'next-inbox',
      priority: 0.7
    })));
...
async function syncMessages(messageIds) {
  if (features.isEnabled('parallel-processing') &&
      features.isEnabled('batch-optimization')) {
...
**Sprint 2 (Weeks 3-4): Core Performance**
4. Batch optimization (100 per batch)
5. Full parallel processing with concurrency control
6. IndexedDB with proper indexes
...
**Sprint 4 (Weeks 7-8): Real-time & Polish**
10. Gmail Pub/Sub notifications
11. Service Worker background sync
12. Predictive prefetching (optional)
...
class SyncTelemetry {
  ...
  getAverages(timeWindow = 3600000) { // 1 hour
