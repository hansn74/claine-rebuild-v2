...
const worker = new Worker(
  new URL('./email-parser.worker.ts', import.meta.url),
  { type: 'module' }
);
...
const sharedWorker = new SharedWorker(
  new URL('./email-sync.shared-worker.ts', import.meta.url),
  { type: 'module' }
);
...
async function processInbox(emails: string[]) {
  const promises = emails.map(rawEmail =>
    emailParserPool.execute('parseEmail', rawEmail)
  );
...
self.addEventListener('message', (event) => {
  ...
  if (type === 'PROCESS_ATTACHMENT') {
    ...
    self.postMessage(
      { type: 'ATTACHMENT_PROCESSED', data: processed },
      [processed]
    );
  }
});
...
class WorkerManager {
  ...
  private handleWorkerCrash(error: ErrorEvent) {
    if (this.restartCount >= this.maxRestarts) {
      throw new Error(
        `Worker crashed ${this.maxRestarts} times, giving up: ${error.message}`
      );
    }
  ...
  async executeWithRetry<T>(
    ...
    for (let i = 0; i <= retries; i++) {
      try {
        ...
        console.warn(
          `Task failed (attempt ${i + 1}/${retries + 1}):`,
          error.message
        );
...
try {
  const result = await workerManager.executeWithRetry(
    'parseEmail',
    { emailId: '123', raw: rawEmail }
  );
} catch (error) {
...
class EmailParser {
  ...
  async parseEmail(
    ...
    const parsed: ParsedEmail = {
      ...
      attachments,
      headers: new Map(
        Object.entries(email.headers || {}).map(([k, v]) => [k, String(v)])
      ),
  ...
  async parseBatch(
    ...
    return Promise.all(
      emails.map((email) => this.parseEmail(email.id, email.raw))
    );
  }
  ...
  constructor(config: SanitizationConfig = {}) {
    this.config = {
      ALLOWED_TAGS: config.allowedTags || [
        'p', 'br', 'div', 'span', 'strong', 'em', 'u', 'a', 'ul', 'ol', 'li',
...
const sanitizedBodies = await sanitizer.sanitizeBatch(
  emails.map((e) => e.html)
);
```
...
class AttachmentProcessor {
  ...
  private async processImage(
    ...
    const scale = Math.min(
      200 / imageBitmap.width,
      200 / imageBitmap.height
    );
...
const attachmentWorker = wrap<AttachmentProcessor>(
  new Worker(
    new URL('./attachment-processor.worker.ts', import.meta.url),
    { type: 'module' }
  )
);
...
async function uploadAttachment(file: File) {
  ...
  // Compress in worker
  const compressed = await attachmentWorker.compressAttachment(
    arrayBuffer,
    file.type
  );
...
export class WorkerManager {
  ...
  async initialize() {
    ...
    const [emailParserPool, searchWorker, attachmentWorker] =
      await Promise.all([
...
export async function initializeApp() {
  ...
  const [, userData, emailList] = await Promise.all([
    workerInit,
    fetchUserData(),
    fetchEmailList(),
  ]);
...
const workerCount = isMobile
  ? Math.min(optimalWorkerCount, 2)
  : optimalWorkerCount;
```
...
const emailParser = {
  ...
  async parseWithFullFeatures(raw: string) {
    ...
    const [{ default: PostalMime }, { default: advanced }] =
      await Promise.all([
        import('postal-mime'),
...
async function loadEmailWithMetrics(emailId: string) {
  ...
  const email = useNewWorkers
    ? await loadEmailV2(emailId)
    : loadEmailV1(emailId);
