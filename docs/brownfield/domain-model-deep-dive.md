...
export class Thread {
  ...
  public getLastMessageId(): string | null {
    return this.messageIds.length > 0
      ? this.messageIds[this.messageIds.length - 1]
      : null;
  }
  ...
  public matchesFilter(filter: ThreadFilter): boolean {
    ...
    if (filter.labels && filter.labels.length > 0) {
      const hasMatchingLabel = filter.labels.some(labelName =>
        this.hasLabel(labelName)
      );
...
export class Label {
   ...
   */
  public isInbox(): boolean { return this.id === 'INBOX'; }
  public isSent(): boolean { return this.id === 'SENT'; }
  public isDraft(): boolean { return this.id === 'DRAFT'; }
  public isTrash(): boolean { return this.id === 'TRASH'; }
  public isSpam(): boolean { return this.id === 'SPAM'; }
  public isStarred(): boolean { return this.id === 'STARRED'; }
}
```
...
export class Workflow {
  ...
  public shouldExecute(thread: Thread): boolean {
    ...
    // Check triggers
    const triggerMatches = this.triggers.some(trigger =>
      this.evaluateTrigger(trigger, thread)
    );
    ...
    return this.conditions.every(condition =>
      this.evaluateCondition(condition, thread)
    );
  }
    ...
    switch (this.operator) {
      ...
      case 'subject_contains':
        return messages.some(msg =>
          msg.subject.toLowerCase().includes(this.value.toLowerCase())
        );
...
export class WorkflowEvaluator {
  ...
  private evaluateTrigger(
    ...
    switch (trigger.type) {
      ...
      case 'from_domain':
        return messages.some(msg =>
          msg.from.isFromDomain(trigger.value)
        );
