 ...
 *
 * Abstract base class for all draft modifiers.
 * Provides common functionality for draft transformations and API calls.
 */
...
/**
 * Abstract base class for draft modifiers
 */
export abstract class BaseDraftModifier implements Modifier<DraftDocument> {
  readonly id: string
  readonly entityId: string
  readonly entityType = 'draft' as const
  readonly createdAt: number
  readonly accountId: string
  readonly provider: ProviderType
  abstract readonly type: DraftModifierType
  ...
  /**
   * Pure function that transforms draft state locally.
   * Must be implemented by subclasses.
   */
  abstract modify(draft: DraftDocument): DraftDocument
  ...
  protected async refreshTokenAndRetry<T>(operation: (token: string) => Promise<T>): Promise<T> {
    ...
    logger.debug('draft-modifier', 'Refreshing token...', {
      provider: this.provider,
      accountId: this.accountId,
    })
