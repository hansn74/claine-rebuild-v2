...
class SessionService {
  ...
  async validateSession(token: string): Promise<boolean> {
    ...
    await db
      .collection('sessions')
      .updateOne({ id: session.id }, { $set: { lastActivityAt: new Date() } })
  ...
  async revokeSession(token: string): Promise<void> {
    ...
    await db
      .collection('sessions')
      .updateOne({ jwtToken: tokenHash }, { $set: { isActive: false } })
  }

  // Revoke all sessions (logout everywhere)
  async revokeAllUserSessions(userId: string): Promise<void> {
    await db.collection('sessions').updateMany({ userId }, { $set: { isActive: false } })
  }
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    const riskScore = await auditLogger.checkSuspiciousActivity(user.id, req.ip)
...
app.post('/api/auth/mfa/verify', authenticateJWT, async (req, res) => {
  ...
  if (verified) {
    await db.collection('users').updateOne({ id: req.user.userId }, { $set: { mfaEnabled: true } })
...
app.get('/api/user/data-export', authenticateJWT, async (req, res) => {
  ...
  userData.note =
    'Email data is stored locally on your device. To export Gmail data, visit Google Takeout.'
