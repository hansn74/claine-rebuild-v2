...
4. **No Memoization** (200ms processing on every render):
   ...
   const uiThreads = useMemo(() => convertToUIThreads(threads), [threads])
   ```
...
const matchedThreads = await db.threads
  .find({
...
const results = threads.filter(
  (t) => t.subject.match(/important.*meeting/i) || t.snippet.match(/important.*meeting/i)
)
...
async function getThreadWithMessages_BAD(threadId: string) {
  ...
  // N+1 PROBLEM!
  const messages = await Promise.all(thread.messageIds.map((id) => db.messages.findOne(id).exec()))
...
async function migrateInBatches(batchSize = 1000) {
  ...
  while (hasMore) {
    // Read batch from v1
    const threads = await v1db.threads.find().skip(processed).limit(batchSize).exec()
...
```typescript
// Before (RxDB):
const unreadInbox = await rxdb.threads
  .find({
    selector: {
      labelIds: { $elemMatch: { $eq: 'INBOX' } },
      isRead: false,
...
async function backgroundMigration() {
  ...
  while (migrated < total) {
    const batch = await v1db.threads.find().skip(migrated).limit(100).exec()
...
// Aggregate query
const unreadCount = await db.threads
  .count({
...
async function getThreadWithMessages(threadId: string) {
  ...
  const messages = await db.messages
    .where('[threadId+date]')
    .between([threadId, 0], [threadId, Number.MAX_SAFE_INTEGER])
    .toArray()
