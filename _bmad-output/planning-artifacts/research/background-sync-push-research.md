...
const setupGmailWatch = async (userId, accessToken) => {
  ...
  const response = await fetch(`https://gmail.googleapis.com/gmail/v1/users/${userId}/watch`, {
    method: 'POST',
    headers: {
...
const processGmailNotification = async (emailAddress, historyId) => {
  ...
  const changes = await fetchHistoryChanges(emailAddress, lastHistoryId, historyId)
      ...
      switch (optimisticUpdate.type) {
        ...
        case 'star':
          return currentEmails.map((email) =>
            email.id === optimisticUpdate.id ? { ...email, starred: !email.starred } : email
          )
        case 'add':
...
class EmailDeduplicator {
  ...
  async processBatch() {
    ...
    const uniqueBatch = Array.from(new Map(batch.map((item) => [item.messageId, item])).values())
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(deduplicator.processEmailUpdate(emailData))
})
```
...
class MessageOrderingQueue {
  ...
  async addMessage(message) {
    ...
    if (historyId === expectedHistoryId + 1) {
      ...
      console.log('Out of order message queued:', {
        threadId,
        historyId,
        expected: expectedHistoryId + 1,
      })
  ...
  async fillHistoryGap(threadId, startHistoryId, endHistoryId) {
    ...
    try {
      ...
      const response = await fetch(
        `/api/history?threadId=${threadId}&start=${startHistoryId}&end=${endHistoryId}`
      )
...
```javascript
const [optimisticEmails, setOptimisticEmails] = useOptimistic(emails, (current, update) => {
