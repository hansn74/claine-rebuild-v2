...
const virtualizer = useVirtualizer({
  ...
  // Auto-measures actual heights after render
  measureElement:
...
function EmailList() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      sessionStorage.setItem('emailListScroll', String(virtualizer.scrollOffset))
    }
...
function EmailList() {
  ...
  useEffect(() => {
    ...
    if (lastItem.index >= allEmails.length - 20 && hasNextPage) {
      fetchNextPage()
    }

    // Also prefetch the page AFTER next
    if (lastItem.index >= allEmails.length - 40 && hasNextPage) {
