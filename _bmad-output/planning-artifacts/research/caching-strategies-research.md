...
async function prefetchSequential(currentIndex: number, emails: Email[]) {
  ...
  const toPrefetch = emails.slice(currentIndex + 1, currentIndex + 1 + prefetchCount)
...
class EmailCacheManager {
  ...
  async invalidateThread(threadId: string) {
    const messages = await this.db.emails.find({ selector: { threadId } }).exec()
...
class CacheInvalidator {
  ...
  async invalidateTag(tag: string) {
    ...
    await Promise.all(Array.from(keys).map((key) => cacheManager.invalidate(key)))
...
class PrefetchManager {
  ...
  private async processPrefetchQueue() {
    while (this.inProgress.size < this.maxConcurrent && this.prefetchQueue.length > 0) {
...
class CacheQuotaManager {
  ...
  private async evict(targetPercent: number) {
    ...
    const emails = await db.emails.find().sort({ lastAccessed: 'asc' }).exec()
